...
function roundingNumber(num:number) {
    let result:number = Math.round(num);
...
function valueNan(val:any):boolean {
  return isNaN(val) // returns true if value is NaN or false otherwise 
}

// Example: Checking if a value is NaN 
